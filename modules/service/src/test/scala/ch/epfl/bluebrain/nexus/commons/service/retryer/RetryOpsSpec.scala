...
import ch.epfl.bluebrain.nexus.commons.service.retryer.RetryOpsSpec.SomeError
...
class RetryOpsSpec
  ...
  "A RetryOps" should {
    "retry a future exponentially when it fails" in new Context {
      retry(future, 3, Backoff(5 seconds, 0)).failed.futureValue(timeout(Span(7500, Milliseconds))) shouldBe SomeError
    ...
    "retry a future exponentially when it fails capped to 1 second" in new Context {
      ...
      future.retry(3).failed.futureValue(timeout(Span(3500, Milliseconds))) shouldBe SomeError
      count.get shouldEqual 4
    }

    "retry a future linearly when it fails" in new Context {
      retry(future, 4, Linear(5 seconds, 300 millis)).failed
        .futureValue(timeout(Span(3500, Milliseconds))) shouldBe SomeError
    ...
    "retry a future linearly when it fails capped to 200 millis" in new Context {
      retry(future, 5, Linear(200 millis, 200 millis)).failed
        .futureValue(timeout(Span(1500, Milliseconds))) shouldBe SomeError
