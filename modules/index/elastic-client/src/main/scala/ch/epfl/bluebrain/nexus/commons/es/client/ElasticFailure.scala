...
object ElasticFailure {
  ...
  /**
    * An unexpected failure when attempting make transformations on the json query
    *
    * @param body   the json where the transformations were attempt
    */
  final case class ElasticUnexpectedTransformationError(body: String)
      extends Err(s"Error while transforming json '$body'")
      with ElasticFailure
