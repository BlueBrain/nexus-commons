...
class ElasticClient[F[_]](base: Uri, queryClient: ElasticQueryClient[F])(implicit
  ...
  def searchChildren[A](query: Json, indices: Set[String] = Set.empty, `type`: String, parentId: String)(
    ...
    queryClient.searchChildren(query, indices, `type`, parentId)(page, fields, sort)
  ...
  /**
    ...
    * @tparam A the generic type to be returned
    */
  def search[A](query: Json, indices: Set[String] = Set.empty, types: Set[String] = Set.empty)(
      page: Pagination,
      fields: Set[Uri] = Set.empty,
      sort: SortList = SortList.Empty)(implicit
                                       rs: HttpClient[F, QueryResults[A]]): F[QueryResults[A]] =
    queryClient(query, indices, types)(page, fields, sort)
}
