package ch.epfl.bluebrain.nexus.service.commons.io

import ch.epfl.bluebrain.nexus.service.commons.io.AkkaCoproductSerializerSpec.Command.TheCommand
import ch.epfl.bluebrain.nexus.service.commons.io.AkkaCoproductSerializerSpec._
  ...
  "An AkkaCoproductSerializer" when {
    ...
    "computing a manifest" should {
      "provide the correct type" in {
        ...
        forAll(mapping.toList) { case (data, manifest) =>
    ...
    "encoding to binary" should {
      "encode known events to UTF-8" in {
        ...
        forAll(mapping.toList) { case (data, out) =>
    ...
    "decoding from binary" should {
      "decode known events" in {
        ...
        forAll(mapping.toList) { case ((json, mf), obj) =>
