...
import ch.epfl.bluebrain.nexus.commons.iam.types.identity.Identity.{AuthenticatedRef, GroupRef, UserRef}
...
class UserInfoSpec extends WordSpecLike with Matchers with Inspectors {
  ...
  "A UserInfo" should {
    ...
    "convert userinfo to user" in {
      val realm = "http://localhost.com/realm"
      model.toUser(realm).identities should contain allElementsOf AuthenticatedUser(
        Set[Identity](AuthenticatedRef(realm)) ++ model.groups.map(GroupRef(realm, _)) + UserRef(realm, model.sub)
      ).identities
